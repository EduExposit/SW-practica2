...
public class TicTacToeHandler extends TextWebSocketHandler {
	@Autowired StadisticsService estructuraDatos;
	...
	public synchronized void handleTextMessage(WebSocketSession session, TextMessage wsMsg) throws Exception {
		...
		try {
			...
			case JOIN_GAME:
				...
				String name = msg.data.name;
				Player player;
				if (estructuraDatos.exists(name)) {
					player = estructuraDatos.getPlayer(name);
					player.setLabel(letter);
					player.setId(numPlayers+1);
				} else {
					player = new Player(numPlayers + 1, letter, msg.data.name);
					estructuraDatos.addPlayer(player);
				}
