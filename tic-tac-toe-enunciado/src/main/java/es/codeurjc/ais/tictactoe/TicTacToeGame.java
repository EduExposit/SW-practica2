...
public class TicTacToeGame {
	...
	public boolean mark(int cellId) {
		...
		if (this.ready && cell.active) {
			...
			if (res.win) {
				...
				winner.player.setJugadas(player.getJugadas()+1);
				player2.setPerdidas(player2.getPerdidas()+1);
				player2.setJugadas(player2.getJugadas()+1);
				winner.pos = res.pos;
				marked.clear();
				...
				player1.setJugadas(player1.getJugadas()+1);
	...
	private void changeTurn() {
		...
		if(this.players.get(this.currentTurn).getName().equals("IA")) {
			CellMarkedValue value = new CellMarkedValue();
			int id=(int) (Math.random()*9);
			while(marked.contains(id)){
				id=(int) (Math.random()*9);
			}
			marked.add(id);
			value.cellId = id;
			value.player = this.players.get(this.currentTurn);
			this.mark(id);
